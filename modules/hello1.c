...
void print_hello(const char *name, unsigned int counter)
{
    ...
    if (!new_event) {
        pr_err("Memory allocation failed for new event entry\n");
        return;
    }
...
{
    pr_info("Service module initialized and running.\n");
    return 0;
}
